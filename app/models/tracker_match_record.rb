  ...
  def self.save_past_match_histories(match_histories, user)
    ...
    history_span.each do |day|
      ...
      results.each do |result|
        ...
        TrackerMatchRecord.create!(user:, match_date: result["date"], legend: result["legend"], kills: result["kills"], damages: result["damages"], wins: result["wins"]) unless TrackerMatchRecord.exists?(user:, match_date: result_date, legend: result["legend"], kills: result["kills"], damages: result["damages"], wins: result["wins"])
      end
    end
  end
