class UsersController < ApplicationController
  ...
  def fetch_trn_overall_stats
    ...
    matches_played_value = @trn_overall_stats["matchesPlayed"][:value]
    kills_value = @trn_overall_stats["kills"][:value]
    wins_value = @trn_overall_stats["wins"][:value]

    @overall_kpm = value_check(matches_played_value, kills_value) do
      TrackerApiService.calculate_kpm(matches_played_value, kills_value)
    end

    @overall_winrate = value_check(matches_played_value, wins_value) do
      TrackerApiService.calculate_winrate(matches_played_value, wins_value)
    end
  end
  ...
  def fetch_trn_current_season_stats
    ...
    @trn_current_season_stats = {}

    @trn_current_season_stats_name.each do |segment|
      @trn_current_season_stats[segment] = {
